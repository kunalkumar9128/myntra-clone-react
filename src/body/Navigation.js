...
const Navigation = ({ cartCount, setSearchel, nevigate, itemArray }) => {
  ...
  function submitSearch(e) {
    ...
    if (!word) {
      ...
      const searched = itemArray.filter((item) => item.title.includes(word));
      setSearchel(searched);
      setWord("");
    }

    nevigate("/product/search");
  }
  const handleEnter = (e) => {
        ...
        <Toolbar disableGutters>
          ...
          <div style={{ display: 'inline-block', textAlign: 'center', marginLeft: '20px' }}>
            <form onSubmit={submitSearch} >
                <input type="text" value={word} onChange={handleEnter} className="form-control" placeholder="Search for Products Branda and More" />
            </form>
          </div>
            ...
            <div style={{ display: 'inline-block', textAlign: 'center', marginLeft: '20px' }}>
              <Link to={"/cart"} style={{ textDecoration: "none", color: "black" }}>
                <LocalMallOutlinedIcon className='m-auto' style={{ marginLeft: '30px', fontSize: '30px' }} /><sup>{cartCount}</sup>
              <p >Bag</p>
              </Link>
            </div>
            {/* </div> */}
